...
export default function TechnicianShow() {
  ...
  const handleTechnicianDelete = React.useCallback(async () => {
    ...
    const confirmed = await dialogs.confirm(
      `Do you wish to delete ${technician.firstName} ${technician.lastName}?`,
      {
    ...
    if (confirmed) {
      ...
      try {
        ...
        notifications.show('Technician deleted successfully.', {
          severity: 'success',
          autoHideDuration: 3000,
        });
      } catch (deleteError) {
        notifications.show(
          `Failed to delete technician. Reason:' ${(deleteError as Error).message}`,
          {
            severity: 'error',
            autoHideDuration: 3000,
          },
  ...
  const pageTitle = `Technician ${technicianId}`;

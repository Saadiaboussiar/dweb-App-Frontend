...
function TechnicianEditForm({
  ...
  const handleFormSubmit = React.useCallback(async () => {
    ...
    try {
      ...
      notifications.show("Technicien modifié avec succès.", {
        severity: "success",
      ...
      notifications.show(
        `Échec de la modification du technicien. Raison : ${
...
export default function TechnicianEdit() {
  ...
  const [tecnician, setTechnician] = React.useState<technicianData | null>(
    null
  );
  ...
  return (
    <PageContainer
      title={`Modifier le technicien ${technicianId}`}
      breadcrumbs={[
        { title: "Techniciens", path: "/technicians" },
        {
          title: `Technicien ${technicianId}`,

...
const ProfilePage = () => {
  ...
  const technicianId: number = Number(sessionStorage.getItem("technicianId"));
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      try {
        await uploadProfilePhoto(file,technicianId);
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        ...
        const data = await getProfileData(technicianId);
        if (data) {
          setProfileData({
  ...
  const handleInputChange = (field: string, value: string) => {
    ...
    const updatedData = editProfileData(technicianId, profileData);
